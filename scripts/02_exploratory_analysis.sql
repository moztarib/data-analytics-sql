...
SELECT 
    ...
    ROUND(SUM(fact.sales), 2) AS generated_revenue,
    ROUND(SUM(fact.profit), 2) AS total_profit,
    ROUND((SUM(fact.profit)/SUM(fact.sales) *100), 2) AS profit_margin_percentage
FROM fact_orders fact
JOIN dim_locations locations ON fact.location_id = locations.location_id
GROUP BY Region
